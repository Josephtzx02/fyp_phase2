...
LEFT_X = -SHELF_WIDTH / 2
BOTTOM_Y = 0.0

BOOK_CLEARANCE = 2.0
FLAT_BOOK_ANGLE = 45.0
...
class Simulator:
    ...
    def __init__(self):
        ...
        self.vis.create_window(
            ...
            width=960, #1280
            height=540, #720
            left=436, #0 = left side
            top=608 #Aligned to bottom
        )
    ...
    def create_robot_base(self):
        ...
        self.base_turntable = turntable

    def get_current_shelf_y(self, target):

        shelf_board = self.shelf_levels[1] # Middle shelf for now
        if abs(target.angle_deg) > FLAT_BOOK_ANGLE:
            vertical_dimension = target.thickness_mm
        else: 
            vertical_dimension = target.height_mm
        return shelf_board + BOARD_THICKNESS + BOOK_CLEARANCE + vertical_dimension / 2
    ...
    def update_target(self, grasp_target):
        ...
        MAX_BASE_SHIFT = 120.0

        self.target_base_x = np.clip(
            calibrated.grasp_x_mm,
            -MAX_BASE_SHIFT,
            MAX_BASE_SHIFT
        )
    ...
    def solve_target(self, target):
        ...
        ik_target = copy.deepcopy(target)
        ik_target.grasp_x_mm -= self.current_base_x
        success, angles = self.ik.solve(ik_target)
    ...
    def update_mobile_base(self):

        dx = self.current_base_x - self.previous_base_x
        ...
        self.base_platform.translate(
            translation,
            relative=True
        )

        self.base_column.translate(
            translation,
            relative=True
        )

        self.base_turntable.translate(
            translation,
            relative=True
        )

        self.vis.update_geometry(self.base_platform)
        self.vis.update_geometry(self.base_column)
        self.vis.update_geometry(self.base_turntable)

        self.previous_base_x = self.current_base_x

        self.robot.set_base_x(
            self.current_base_x
        )
    ...
    def update(self):
        if self.current_target is not None:
            ...
            if self.robot_state == STATE_APPROACH:
                ...
                goal = self.current_target.grasp_z_mm - self.current_target.width_mm - 30.0 
        ...
        if abs(base_error) > 0.5:
            base_step = np.clip(
                base_error,
                -self.base_speed,
                self.base_speed
            )
            self.current_base_x += base_step
