...
while True:
    ...
    if selected_det:
        ...
        if selected_det["grasp_target"]:
            ...
            lines.extend([
                "----- Grasp Target -----",
                f"Target X    : {gt.x_mm:.1f} mm" if gt else "Target X    : N/A",
                f"Target Y    : {gt.y_mm:.1f} mm" if gt else "Target Y    : N/A",
                f"Target Z    : {gt.z_mm:.1f} mm" if gt else "Target Z    : N/A",
                f"Grip Angle  : {gt.angle_deg:.1f} deg" if gt else "Grip Angle  : N/A"
            ])
